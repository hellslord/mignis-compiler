...
def main():
    ...
    if len(sys.argv) == 1 or (len(sys.argv) < 3 and sys.argv[1] != "list"):
        print("Usage: ./target_compiler.py [list | <target_language>] <directory>")
        exit(-1)

    # We save the directory we want to work on and we check that the directory name is well-formed
    ...
    if main_dir != "":
        try:
            # If <dir>/final already exists, we delete it. We remove it if it is a file or a dir as well
            dir = main_dir + "/final"
    ...
    elif sys.argv[1] == "list":  # Special value: we obtain a list of supported target languages
        print("List of supported final target languages:")
        print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
        print("\n")
        print("IPTABLES:\tStandard Netfilter/iptables for Linux OS")
        print("EXAMPLE:\tAn example used as test that produces a fake final configuration")
        print("\n")
        exit(0)
    else:  # Unknown language
        print("Unknown language. Type './target_compiler list' for the complete list of supported target languages")
        exit(1)
